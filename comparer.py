def read_file_line_by_line(file_path):
    data_types = ['int', 'float', 'string', 'boolean', 'list', 'dict', 'tuple', 'set', 'nested', 'custom_obj', 'recursive_list']
...
def comparer(hash_lists):
    for data_type, hashes in hash_lists.items():
        if data_type in ['int', 'float', 'string', 'boolean', 'list', 'dict', 'tuple', 'set', 'nested', 'custom_obj', 'recursive_list']:
            if len(hashes) > 1:
                first_list = hashes[0]
